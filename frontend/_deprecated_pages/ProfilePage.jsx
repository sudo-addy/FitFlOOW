...
export default function ProfilePage() {
  const { user: globalUser, isSidebarCollapsed, updateUser } = useAuth();
  const [user, setUser] = useState(() => ({
    name: globalUser?.name || 'Marcus Vance',
    email: globalUser?.email || 'marcus.vance@elite.com',
    phone: globalUser?.phone || '+1 (555) 382-9001',
    tier: globalUser?.tier || 'Warrior Elite',
    powerLevel: globalUser?.powerLevel || 8965,
    streak: globalUser?.streak || 5,
    workouts: globalUser?.workouts || 28,
    classes: globalUser?.classes || 12,
    prs: globalUser?.prs || 9,
    longestStreak: globalUser?.longestStreak || 14,
    height: globalUser?.height || 182,
    weight: globalUser?.weight || 81.6,
    targetWeight: globalUser?.targetWeight || 80.0,
    publicLeaderboard: globalUser?.publicLeaderboard ?? true,
    allowCoachView: globalUser?.allowCoachView ?? true,
    profileVisibility: globalUser?.profileVisibility || 'Public',
    classReminders: globalUser?.classReminders ?? true,
    streakReminders: globalUser?.streakReminders ?? true,
    newProgramReminders: globalUser?.newProgramReminders ?? true,
    coachMessagesReminders: globalUser?.coachMessagesReminders ?? true,
  }));
